...
#     /* Grade badges */
#     .badge-A { background:#dcfce7; color:#15803d; padding:4px 14px; border-radius:20px; font-size:14px; font-weight:600; }
#     .badge-B { background:#dbeafe; color:#1d4ed8; padding:4px 14px; border-radius:20px; font-size:14px; font-weight:600; }
#     .badge-C { background:#fef9c3; color:#a16207; padding:4px 14px; border-radius:20px; font-size:14px; font-weight:600; }
#     .badge-D { background:#fee2e2; color:#b91c1c; padding:4px 14px; border-radius:20px; font-size:14px; font-weight:600; }
#     .badge-F { background:#f3e8ff; color:#7e22ce; padding:4px 14px; border-radius:20px; font-size:14px; font-weight:600; }
...
# GRADE_COLORS  = {0: '#7c3aed', 1: '#dc2626', 2: '#d97706', 3: '#2563eb', 4: '#16a34a'}
# GRADE_MEANING = {
#     0: 'Failing — needs immediate support',
#     1: 'Below average — significant improvement needed',
#     2: 'Average — meeting basic expectations',
#     3: 'Good — above average performance',
#     4: 'Excellent — top performance',
# }
# GRADE_GPA_RANGE = {0: '< 1.0', 1: '1.0 – 1.9', 2: '2.0 – 2.9', 3: '3.0 – 3.9', 4: '≥ 4.0'}

# PLOT_COLORS = ['#7c3aed', '#dc2626', '#d97706', '#2563eb', '#16a34a']
...
# def train_models(df):
#     X = df[FEATURES]
#     y_cls = df['GradeClass'].astype(int)
#     y_reg = df['GPA']
...
#     reg = Pipeline([('scaler', StandardScaler()), ('knn', KNeighborsRegressor(n_neighbors=5))])
...
#     pi = permutation_importance(clf, X_test, yc_test, n_repeats=10, random_state=42)
#     fi = dict(zip(FEATURES, pi.importances_mean.tolist()))

#     cm = confusion_matrix(yc_test, yc_pred)

#     return clf, reg, X_train, yc_train, yr_train, X_test, yc_test, yr_test, acc, mse, r2, fi, cm
...
#         st.markdown("KNN · 2,392 students · 12 features")
#         st.divider()
...
#         ped    = st.selectbox("Parental education", ["None", "High school", "Some college", "Bachelor's", "Higher"], index=2)

#         st.markdown("##### Academic")
#         study    = st.slider("Study time (hrs/week)", 0.0, 20.0, 10.0, step=0.5)
#         absences = st.slider("Absences (days)", 0, 29, 8)
#         tutoring = st.selectbox("Tutoring", ["No", "Yes"])
#         psupport = st.selectbox("Parental support", ["None", "Low", "Moderate", "High", "Very high"], index=2)
...
#         st.divider()
#         st.markdown("<div style='font-size:11px; color:#475569'>Model: KNN (k=5) · StandardScaler pipeline</div>", unsafe_allow_html=True)
...
# # ── Predict Tab ───────────────────────────────────────────────────────────────
# def render_predict_tab(inputs, predict_btn, clf, reg, X_train, yc_train, yr_train, acc, mse, r2):
#     # Top metrics
#     c1, c2, c3, c4 = st.columns(4)
#     with c1:
#         st.markdown(metric_card("Total students", "2,392", "in training dataset"), unsafe_allow_html=True)
#     with c2:
#         st.markdown(metric_card("KNN accuracy", f"{acc*100:.1f}%", "grade classification"), unsafe_allow_html=True)
#     with c3:
#         st.markdown(metric_card("GPA R² score", f"{r2:.3f}", "regression performance"), unsafe_allow_html=True)
#     with c4:
#         st.markdown(metric_card("K neighbors", "5", "used per prediction"), unsafe_allow_html=True)
...
#             grade_pred = int(clf.predict(input_df)[0])
#             gpa_pred   = float(reg.predict(input_df)[0])
#             gpa_pred   = round(min(4.0, max(0.0, gpa_pred)), 2)

#             # nearest neighbors
#             scaler  = clf.named_steps['scaler']
#             X_sc    = scaler.transform(X_train)
#             inp_sc  = scaler.transform(input_df)
#             dists   = np.sqrt(((X_sc - inp_sc)**2).sum(axis=1))
#             top5    = np.argsort(dists)[:5]

#             st.session_state['last_prediction'] = {
#                 'grade': grade_pred, 'gpa': gpa_pred,
#                 'neighbors': [(i, dists[i], int(yc_train.iloc[i]), float(yr_train.iloc[i])) for i in top5],
#                 'inputs': inputs
#             }

#         pred = st.session_state['last_prediction']
#         grade, gpa = pred['grade'], pred['gpa']
#         gl = GRADE_LABELS[grade]
#         gc = GRADE_COLORS[grade]

#         col_res, col_nb = st.columns([3, 2])
#         with col_res:
#             st.markdown('<div class="result-card">', unsafe_allow_html=True)
#             st.markdown('<div class="result-title">Prediction result</div>', unsafe_allow_html=True)

#             r1, r2_ = st.columns([1, 1])
#             with r1:
#                 st.markdown(f"""
#                 <div style="margin-bottom:16px;">
#                     <div style="font-size:12px;color:#64748b;margin-bottom:6px;font-weight:500;">Predicted GPA</div>
#                     <div style="font-size:36px;font-weight:700;color:#0f172a;line-height:1">{gpa:.2f}</div>
#                     <div style="font-size:12px;color:#94a3b8;margin-top:2px;">out of 4.00</div>
#                 </div>
#                 """, unsafe_allow_html=True)
#             with r2_:
#                 st.markdown(f"""
#                 <div style="margin-bottom:16px;">
#                     <div style="font-size:12px;color:#64748b;margin-bottom:6px;font-weight:500;">Grade class</div>
#                     <span class="badge-{gl}" style="background:{gc}22;color:{gc}">{gl} — Class {grade}</span>
#                     <div style="font-size:12px;color:#94a3b8;margin-top:6px;">GPA range: {GRADE_GPA_RANGE[grade]}</div>
#                 </div>
#                 """, unsafe_allow_html=True)
...
#                     <div style="width:{pct:.1f}%;height:100%;background:{gc};border-radius:5px;transition:width 0.5s;"></div>
#                 </div>
...
#                 <div style="font-size:12px;color:#64748b;margin-bottom:4px;font-weight:500;">Performance assessment</div>
#                 <div style="font-size:14px;color:#0f172a;">{GRADE_MEANING[grade]}</div>
#             </div>
#             """, unsafe_allow_html=True)

#             # Key risk/boost factors
#             abs_val = pred['inputs']['Absences']
#             study_val = pred['inputs']['StudyTimeWeekly']
#             tut_val = pred['inputs']['Tutoring']
#             factors = []
#             if abs_val > 20:
#                 factors.append(("⚠️ High absences", f"{abs_val} days — major risk factor", "#fef9c3", "#92400e"))
#             elif abs_val < 5:
#                 factors.append(("✅ Low absences", f"Only {abs_val} days — positive indicator", "#f0fdf4", "#15803d"))
#             if study_val > 14:
#                 factors.append(("✅ Strong study habits", f"{study_val:.1f} hrs/week", "#f0fdf4", "#15803d"))
#             if tut_val:
#                 factors.append(("✅ Tutoring support", "Active tutoring engaged", "#eff6ff", "#1d4ed8"))

#             if factors:
#                 for icon, text, bg, color in factors[:2]:
#                     st.markdown(f"""
#                     <div style="background:{bg};border-radius:8px;padding:10px 14px;margin-top:8px;font-size:13px;color:{color};">
...
#         with col_nb:
#             st.markdown('<div class="result-card">', unsafe_allow_html=True)
#             st.markdown('<div class="result-title">5 nearest neighbors</div>', unsafe_allow_html=True)
#             st.markdown('<div style="font-size:12px;color:#94a3b8;margin-bottom:12px;">Most similar students from training data</div>', unsafe_allow_html=True)

#             for rank, (idx, dist, nb_grade, nb_gpa) in enumerate(pred['neighbors'], 1):
#                 gl_nb = GRADE_LABELS[nb_grade]
#                 gc_nb = GRADE_COLORS[nb_grade]
#                 st.markdown(f"""
#                 <div class="neighbor-row">
#                     <div style="display:flex;align-items:center;gap:10px;">
#                         <div class="neighbor-idx">{rank}</div>
#                         <div>
#                             <div style="font-size:13px;font-weight:500;color:#0f172a;">Student #{idx+1}</div>
#                             <div style="font-size:11px;color:#94a3b8;">dist: {dist:.3f} · GPA: {nb_gpa:.2f}</div>
#                         </div>
#                     </div>
#                     <span style="background:{gc_nb}22;color:{gc_nb};padding:3px 10px;border-radius:12px;font-size:12px;font-weight:600;">{gl_nb}</span>
#                 </div>
#                 """, unsafe_allow_html=True)

#             # Neighbor grade distribution
#             nb_grades = [g for _, _, g, _ in pred['neighbors']]
#             grade_counts = {gl: nb_grades.count(g) for g, gl in GRADE_LABELS.items()}
#             st.markdown("<div style='margin-top:16px;'>", unsafe_allow_html=True)
#             fig_nb = go.Figure(go.Bar(
#                 x=list(grade_counts.keys()),
#                 y=list(grade_counts.values()),
#                 marker_color=[GRADE_COLORS[g] for g in GRADE_LABELS.keys()],
...
#     else:
#         st.markdown("""
#         <div style="background:white;border-radius:12px;padding:48px;border:1px solid #e2e8f0;text-align:center;">
#             <div style="font-size:48px;margin-bottom:12px;">🎯</div>
#             <div style="font-size:18px;font-weight:600;color:#0f172a;margin-bottom:8px;">Ready to predict</div>
#             <div style="font-size:14px;color:#64748b;">Configure the student profile in the sidebar and click <strong>Predict performance</strong></div>
#         </div>
#         """, unsafe_allow_html=True)
...
#             x=[GRADE_LABELS[int(g)] for g in grade_counts.index],
...
#             title=dict(text="Grade class distribution", font=dict(size=14, family='DM Sans')),
...
#     with col2:
#         st.markdown('<div class="chart-card">', unsafe_allow_html=True)
#         fig_gpa = go.Figure(go.Histogram(
#             x=df['GPA'], nbinsx=30,
#             marker_color='#3b82f6', opacity=0.8,
#         ))
#         fig_gpa.update_layout(
#             title=dict(text="GPA distribution", font=dict(size=14, family='DM Sans')),
#             height=300, margin=dict(l=0, r=0, t=40, b=0),
#             showlegend=False, plot_bgcolor='white', paper_bgcolor='white',
#             yaxis=dict(showgrid=True, gridcolor='#f1f5f9'),
#             xaxis=dict(showgrid=False, title='GPA'),
#             font=dict(family='DM Sans'),
#         )
#         st.plotly_chart(fig_gpa, use_container_width=True, config={'displayModeBar': False})
#         st.markdown('</div>', unsafe_allow_html=True)
...
#     fi_colors = ['#16a34a' if v >= 0 else '#dc2626' for v in fi_vals]
...
#         title=dict(text="Feature importance (permutation-based)", font=dict(size=14, family='DM Sans')),
...
#     st.markdown("""
#     <div class="info-box">
#         <strong>Absences</strong> is the dominant predictor (+22.1% impact). 
#         Green bars indicate features that improve prediction accuracy when permuted; red bars indicate features with negative/neutral contribution.
#     </div>
#     """, unsafe_allow_html=True)
#     st.markdown('</div>', unsafe_allow_html=True)
...
#         fig_sc = px.scatter(
#             sample, x='Absences', y='GPA',
#             color='GradeClass',
#             color_continuous_scale=['#7c3aed','#dc2626','#d97706','#2563eb','#16a34a'],
#             opacity=0.6, title='Absences vs GPA',
#         )
#         fig_sc.update_layout(
#             height=300, margin=dict(l=0,r=0,t=40,b=0),
#             plot_bgcolor='white', paper_bgcolor='white',
#             font=dict(family='DM Sans'),
#             coloraxis_colorbar=dict(title='Grade'),
#         )
...
#         st.markdown('<div class="chart-card">', unsafe_allow_html=True)
#         fig_st = px.scatter(
#             sample, x='StudyTimeWeekly', y='GPA',
#             color='GradeClass',
#             color_continuous_scale=['#7c3aed','#dc2626','#d97706','#2563eb','#16a34a'],
#             opacity=0.6, title='Study time vs GPA',
#         )
...
# # ── Model Info Tab ────────────────────────────────────────────────────────────
# def render_model_tab(clf, acc, mse, r2, cm):
#     c1, c2, c3 = st.columns(3)
#     with c1:
#         st.markdown(metric_card("Algorithm", "KNN", "K-Nearest Neighbors"), unsafe_allow_html=True)
#     with c2:
#         st.markdown(metric_card("Pipeline", "2-step", "StandardScaler → KNN"), unsafe_allow_html=True)
#     with c3:
#         st.markdown(metric_card("Train / test", "80 / 20", "1,913 / 479 samples"), unsafe_allow_html=True)
...
#             title=dict(text="Confusion matrix (grade class)", font=dict(size=14, family='DM Sans')),
...
#     with col2:
#         st.markdown('<div class="result-card">', unsafe_allow_html=True)
#         st.markdown('<div class="result-title">Model parameters</div>', unsafe_allow_html=True)
...
#         metrics = [
#             ("Classification accuracy", f"{acc*100:.1f}%"),
#             ("GPA MSE", f"{mse:.3f}"),
#             ("GPA R² score", f"{r2:.3f}"),
#         ]
#         for k, v in metrics:
...
#     st.markdown('<div class="warn-box">The original <code>.pkl</code> file was trained on a different scikit-learn version. The model above was retrained on the same dataset with identical hyperparameters, achieving matching performance metrics.</div>', unsafe_allow_html=True)
...
#     df = load_data()
#     clf, reg, X_train, yc_train, yr_train, X_test, yc_test, yr_test, acc, mse, r2, fi, cm = train_models(df)
...
#         <h1 style="font-size:28px;font-weight:700;color:#0f172a;margin-bottom:4px;">🎓 Student Performance Predictor</h1>
#         <p style="font-size:15px;color:#64748b;margin:0;">K-Nearest Neighbors model · Predict GPA and grade class from student attributes</p>
#     </div>
#     """, unsafe_allow_html=True)
...
#     with tab1:
#         render_predict_tab(inputs, predict_btn, clf, reg, X_train, yc_train, yr_train, acc, mse, r2)

#     with tab2:
#         render_insights_tab(df, fi)

#     with tab3:
#         render_model_tab(clf, acc, mse, r2, cm)
    ...
    # with c1:
        ...
        # fig = go.Figure(go.Bar(
            ...
            # marker=dict(color=clrs, opacity=0.8,
                        # line=dict(color=[c+'55' for c in clrs], width=1)),
            # text=grade_cts.values, textposition='outside',
            # textfont=dict(size=11, color="#04776A"),
        # ))
        # fig.update_layout(
            # title=dict(text='Grade Distribution', font=dict(size=14,color="#6A1212",family=FONT)),
...
# 
    # fig3 = go.Figure(go.Bar(
        ...
        # marker=dict(color=clrs_fi, opacity=0.8,
                    # line=dict(color=[c+'55' for c in clrs_fi], width=0.8)),
...
# 
    # with cl:
        ...
        # )
        # def _cm_chart():
            # st.plotly_chart(fig_cm, use_container_width=True, config=CHART_CFG)
...
# 
    ...
    # mse = mean_squared_error(yr_te, reg.predict(X_te))
    # r2  = r2_score(yr_te, reg.predict(X_te))
# 
    # cm  = confusion_matrix(yc_te, clf.predict(X_te))
# 
    # return clf, reg, X_tr, yc_tr, yr_tr, acc, mse, r2, cm
# 
# ---------------- MAIN ----------------
# df = load_data()
# clf, reg, X_tr, yc_tr, yr_tr, acc, mse, r2, cm = train_models(df)
# 
# st.title("⚡ EduPredict AI")
# 
# ---------------- INPUT ----------------
# age = st.slider("Age", 15, 18, 17)
# study = st.slider("Study Time", 0.0, 20.0, 10.0)
...
# 
    # gc = GRADE_COLORS[grade]
# 
    # st.success(f"Predicted Grade: {GRADE_LABELS[grade]}")
    # st.info(f"Predicted GPA: {gpa:.2f}")
# 
    # ---------------- RADAR FIX ----------------
    # fig_r = go.Figure(go.Scatterpolar(
        ...
        # fill='toself',
        # fillcolor=hex_to_rgba(gc, 0.08),   # FIXED
        # line=dict(color=gc)
    # ))
# 
...
# fig = go.Figure(go.Bar(
    ...
    # marker=dict(
        # color=clrs,
        # line=dict(color=[hex_to_rgba(c, 0.3) for c in clrs])  # FIXED
    # )
# ))
# 
...
# fi = permutation_importance(clf, df[FEATURES], df['GradeClass'], n_repeats=5)
# fi_vals = fi.importances_mean
# clrs_fi = ['#10b981' if v >= 0 else '#f43f5e' for v in fi_vals]
# 
# fig2 = go.Figure(go.Bar(
    ...
    # marker=dict(
        # color=clrs_fi,
        # line=dict(color=[hex_to_rgba(c, 0.3) for c in clrs_fi])  # FIXED
    # )
# ))
# 
